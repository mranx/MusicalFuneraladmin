...
export async function sendOrderNotification(email: string, orderData: any) {
  const mailOptions = {
    from: `"Music Funeral" <${process.env.EMAIL_FROM}>`,
    to: email,
    subject: "Order Confirmation - Music Funeral",
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2>Order Confirmation</h2>
        <p>Dear ${orderData.personName},</p>
        <p>Thank you for placing your order with us. Below are the details:</p>
        
        <h3>Order Details</h3>
        <p><strong>Order ID:</strong> ${orderData.orderId}</p>
        <p><strong>Deceased Name:</strong> ${orderData.deceasedName}</p>
        <p><strong>Relation:</strong> ${orderData.relation}</p>
        <p><strong>Service Date:</strong> ${new Date(orderData.serviceDate).toLocaleDateString()}</p>
        <p><strong>Payment Status:</strong> ${orderData.paymentStatus}</p>
        
        <p>We will contact you soon for further details.</p>
        <p>Best Regards,<br>Music Funeral Team</p>
      </div>
    `,
  };
